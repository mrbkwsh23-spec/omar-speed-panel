...
<head>
    ...
    <style>
        :root { --neon-color: #00f0ff; --neon-glow: rgba(0, 240, 255, 0.4); --dynamic-ring: #39ff14; }
        body { background-color: #05070c; color: #ffffff; font-family: sans-serif; display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100vh; margin: 0; overflow: hidden; }
        body.hud-mode { transform: scaleX(-1); }
        h1 { color: var(--neon-color); text-shadow: 0 0 10px var(--neon-color); text-align: center; font-weight: 800; font-size: 1.2rem; margin-bottom: 5px; }
        ...
        .gauge { width: 100%; height: 100%; border-radius: 50%; background: conic-gradient(var(--dynamic-ring) 0deg, #ff007f 180deg, var(--dynamic-ring) 240deg, #121824 240deg); display: flex; align-items: center; justify-content: center; box-shadow: 0 0 25px var(--neon-glow); }
        ...
        .needle { position: absolute; width: 4px; height: 95px; background: linear-gradient(to top, transparent 20%, #ff0055 20%); bottom: 50%; left: calc(50% - 2px); transform-origin: bottom center; transform: rotate(-120deg); transition: transform 0.1s ease-out; filter: drop-shadow(0 0 4px #ff0055); }
